...
`fzfsym` is a fuzzy symbol search tool. Treat it as a discovery layer: it points you to likely symbols, but you still need to inspect the code before deciding whether something is reusable.
...
3. Run `fzfsym` against the narrowest useful root first, then broaden if needed.
...
When `fzfsym` is installed in the target project:

```bash
fzfsym "normalize path" src --kind function,method,constant,type --limit 20
fzfsym "score" src --kind function,method --format json --limit 15
```

When working inside this `fzfsym` repository:
...
```bash
bunx fzfsym "parse config" src --kind function,method,type --limit 20
```

If `fzfsym` cannot be run, say so and fall back to the repository's available search tools. Still follow the same deduplication intent: search before implementing.
...
```bash
fzfsym "<query>" <root> --kind function,method,constant,type,interface --limit 20
```

Use JSON output when you need structured fields for ranking, locations, signatures, or automation:

```bash
fzfsym "<query>" <root> --format json --limit 20
```
...
```markdown
I checked for existing symbols with `fzfsym` using queries like `normalize`, `path`, and `resolve`. The closest match was `normalizePath` in `src/files.ts`, so I reused that behavior instead of adding a separate helper.
```

If nothing relevant exists:

```markdown
I searched with `fzfsym` for `parse`, `config`, and `loadConfig` across `src/`; the matches were unrelated, so I added a new helper in the nearest module.
```
